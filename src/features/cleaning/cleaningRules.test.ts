...
describe('cleaningRules', () => {
  ...
  it('keeps a newly adopted or freshly cleaned rock visually clean for twelve hours', () => {
    expect(getDustAmount(null, adoptedAt, adoptedMs + DUST_VISIBLE_AFTER_MS)).toBe(0)
    expect(hasVisibleDust(0)).toBe(false)
  })

  it('accumulates dust progressively and caps it after fourteen days', () => {
  ...
  it('uses last_cleaned_at as the new accumulation baseline', () => {
    ...
    expect(getDustAmount(cleanedAt, adoptedAt, cleanedMs + 60 * 60 * 1000)).toBe(0)
  })
