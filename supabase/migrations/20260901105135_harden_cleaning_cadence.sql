-- CAILLOU™ step 09: keep cleaning statistics meaningful and idempotent.
-- A surface only accumulates visible dust after twelve hours, so distinct
-- cleaning events are rejected during the same interval. Exact event-key
-- replays still return their stored receipt before this guard is evaluated.
...
begin
  ...
  if now() - coalesce(v_previous_cleaned_at, v_adopted_at) < interval '12 hours' then
