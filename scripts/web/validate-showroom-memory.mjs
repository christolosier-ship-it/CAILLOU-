...
try {
  const response = await page.goto(`${baseUrl}/scripts/web/showroom-memory-validation.html`, {
    waitUntil: 'domcontentloaded',
    timeout: 20_000,
  })
  if (!response?.ok()) throw new Error(`Validation page returned HTTP ${response?.status() ?? 'unknown'}`)
  ...
  const verdict = await page.evaluate(() => ({
    ...
    report: window.__SHOWROOM_REPORT__ ?? null,
  }))
  ...
  await page.screenshot({ path: `${outputDir}/showroom-validation.png`, type: 'png', fullPage: false })

  if (verdict.status !== 'pass') throw new Error(`Showroom GPU validation failed: ${verdict.error || 'unknown error'}`)

  console.log(`[CAILLOU] showroom GPU PASS: ${verdict.report.rounds} rounds, ${verdict.report.rocksPerRound} rocks/round`)
