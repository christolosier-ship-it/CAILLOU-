...
import type {
  CaressMutationResult,
  RegisterCaressMutation,
  RockEconomySnapshot,
} from '../../src/features/caress/caressTypes'
...
function CaressFixture() {
  ...
  const receipts = useRef(new Map<string, CaressMutationResult>())
  ...
  const mutation: RegisterCaressMutation = useCallback(async ({ eventKey }) => {
    ...
    const next: CaressMutationResult = {
    ...
    serverEconomy.current = { ...serverEconomy.current, ...next }
